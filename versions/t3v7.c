...
#define RAIO_LIM 10 /* diametro>=10 e circulo */
#define FORMATO_LIM 10 /* limite da diferenca entre os raios pra considerar circulo */
#define AREA_LIM 20
...
{
    unsigned int i, j, k, cor, tem_branco = 1, cont = 0, a = 0,contador, figuras, m, n, larg, alt, area_pixels, **copia;
...
//    destroiImagem1C(img);
    ...
    for(cor = COR_INICIAL; cor<=figuras; cor++)// mede a maior dimensao horizontal, vertical e diagonal de cada figura
    {
        ...
        {
            area = M_PI*pow((vertical+horizontal)/4.0, 2);
            if(diagonal >= sqrt(vertical*vertical + horizontal*horizontal) - DIM_LIM)
            {
                for(m=0;m<alt;m++)
                    for(n=0;n<larg;n++)
                    {
            ...
            else if(area_pixels - area >= -AREA_LIM && area_pixels - area <= AREA_LIM)
            {
                for(m=0;m<alt;m++)
                    for(n=0;n<larg;n++)
                    {
...
//            area = pow((vertical+diagonal+horizontal)/3.0, 2);
//            else if(area_pixels - area >= -AREA_LIM && area_pixels - area <= AREA_LIM && )
//            {
//                for(m=0;m<alt;m++)
//                    for(n=0;n<larg;n++)
...
//                printf(" retangulo");
//            }
            else if((vertical - horizontal) <= FORMATO_LIM && (vertical - horizontal) >= -FORMATO_LIM && (vertical - diagonal) <= FORMATO_LIM && (vertical - diagonal) >= -FORMATO_LIM && (horizontal - diagonal) <= FORMATO_LIM && (horizontal - diagonal) >= -FORMATO_LIM)
            {
                for(m=0;m<alt;m++)
                    for(n=0;n<larg;n++)
                    {
            ...
            {
                if((horizontal >= RETANG_LIM_HORI && vertical >= RETANG_LIM_VERT) || (horizontal >= RETANG_LIM_VERT && vertical >= RETANG_LIM_HORI))
                {
                    for(m=0;m<alt;m++)
                        for(n=0;n<larg;n++)
                        {
