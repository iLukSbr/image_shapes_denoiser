...
#define FORMATO_LIM 6 /* limite da diferenca entre os raios pra considerar circulo */
#define DISPARIDADE 2.48
...
{
    unsigned int i, j, k, cor, tem_branco = 1, cont = 0, a = 0,contador, figuras, m, n, larg, alt, **copia,maior;
    int horizontal, vertical, diagonal;
    float area_circular,diagonal_quadrado;
        ...
        for(j=0;j<img->largura;j++)
        {
            if(copia[i][j]==BRANCO)
            {
               ...
               //printf("\nCor = %u ", cor);
                ...
                a=0;
                cont=0;
               // salvaImagem1C(img,"coloridinho.bmp");
            }
        }
    }
...
//    destroiImagem1C(img);
    ...
    for(cor = COR_INICIAL; cor<=figuras; cor++)// mede a maior dimensao horizontal, vertical e diagonal de cada figura
    {
        ...
        diagonal_quadrado=sqrt((vertical*vertical)+(horizontal*horizontal))/2;
        //printf("\nf%u h%u v%u d%u ml%f dq%f", cor, horizontal, vertical, diagonal,media_lado,diagonal_quadrado);

        /*contagem das figuras por tipo*/

        if(((vertical - horizontal) >= FORMATO_LIM )|| ((vertical - horizontal) <= -FORMATO_LIM)||
           ((DISPARIDADE<=(diagonal-diagonal_quadrado))||((diagonal-diagonal_quadrado)<=-DISPARIDADE)))
        {
                for(m=0;m<alt;m++)
                    for(n=0;n<larg;n++)
                    {
        ...
        /*if((vertical - horizontal) <= FORMATO_LIM && (vertical - horizontal) >= -FORMATO_LIM && (vertical - diagonal) <= FORMATO_LIM && (vertical - diagonal) >= -FORMATO_LIM && (horizontal - diagonal) <= FORMATO_LIM && (horizontal - diagonal) >= -FORMATO_LIM)
        {
            ...
            //if((diagonal-diagonal_quadrado<=DISPARIDADE)&&(diagonal-diagonal_quadrado)>=-DISPARIDADE)
            {
                for(m=0;m<alt;m++)
                    for(n=0;n<larg;n++)
                    {
